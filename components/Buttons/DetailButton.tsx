    ...
    <ButtonGroup>
      {(!cartItem) ? (
        <>
          ...
          <Favorite onClick={() => (navigator.share({ title: product.title, text: `Encontrei esse ${product.title} no site da Tecdata! Confere clicando nesse link"`, url: window.location.href }))} ><FaShare size={16} color={storeData.primaryColor} /></Favorite>
        </>
      ) : (
        <>
